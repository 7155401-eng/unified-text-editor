...
const SERVICE = "ravtext-cloudflare-docx-api";
const VERSION = "2026-05-25-diagnostics";
...
function cors(id = "") {
  ...
  if (id) h["x-docx-request-id"] = id;
  return h;
}

export function jsonResponse(payload, status = 200, id = "") {
...
export const optionsResponse = (id = "") => new Response(null, { status: 204, headers: cors(id) });
export const methodNotAllowed = (id = "") => jsonResponse({ ok: false, serverSide: true, requestId: id, error: "Method not allowed" }, 405, id);
export const routeNotImplemented = (kind = "extract", id = "") => jsonResponse({
  ok: false,
  serverSide: true,
  requestId: id,
...
const norm = (v) => String(v || "").normalize("NFD").replace(HEBREW_MARKS_RE, "").trim().toLowerCase();
...
function levelOfParagraph(pXml, styles) {
  ...
  for (let i = 1; i <= 6; i += 1) {
    if (norm(styleId) === String(i) || marker.includes(`heading ${i}`) || marker.includes(`heading${i}`) || marker.includes(`כותרת ${i}`) || marker.includes(`כותרת${i}`)) return i;
  }
  return 0;
}
...
function splitDocumentXml(xml) {
  ...
  return {
    bodyXml: String(xml).slice(open.index + open[0].length, closeIndex),
  };
}
...
export async function importDocxArrayBuffer(arrayBuffer, id = "") {
...
export async function handleImportRequest(request) {
  const id = request.headers.get("x-docx-request-id") || rid();
  ...
  if (request.method === "GET") {
    return jsonResponse({
      ...
      requestId: id,
      path: url.pathname,
      message: "Cloudflare DOCX API route is active.",
      routes: ["/api/word-chapters-import", "/api/word-chapters/import", "/api/word-chapters-scan", "/api/word-chapters/scan"],
    }, 200, id);
  }
  ...
  try {
    ...
    const imported = await importDocxArrayBuffer(arrayBuffer, id);
