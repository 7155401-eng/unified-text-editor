(() => {
  ...
  const RESET_PREFIXES = ['ravtext.talmudLayout.','ravtext.mishnaWrap.','ravtext.v9.','ravtext.layout.','ravtext.liveOverflow.'];
  ...
  function installStyle() {
    ...
    s.textContent = '#btn-render-tab{font-weight:700}.render-safety-toolbar{direction:rtl}.render-safety-toolbar .tb-group button{white-space:nowrap}.render-safety-toast{position:fixed;left:18px;bottom:18px;z-index:2147483100;background:#111827;color:#fff;padding:10px 14px;border-radius:10px;box-shadow:0 8px 22px rgba(0,0,0,.24);direction:rtl}.render-safety-modal-backdrop{position:fixed;inset:0;z-index:2147483050;background:rgba(15,23,42,.55);display:flex;align-items:center;justify-content:center;padding:20px}.render-safety-modal{direction:rtl;width:min(780px,94vw);max-height:88vh;overflow:auto;background:#fff;color:#111827;border-radius:14px;box-shadow:0 12px 34px rgba(15,23,42,.24);padding:20px 22px;font-family:Segoe UI,system-ui,sans-serif;line-height:1.55}.render-safety-head{display:flex;align-items:center;justify-content:space-between;gap:12px}.render-safety-head h2{margin:0;font-size:19px}.render-safety-close{border:0;background:transparent;cursor:pointer;font-size:24px;color:#64748b}.render-safety-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.render-safety-card{border:1px solid #dbeafe;border-radius:10px;padding:10px 12px;background:#f8fafc;margin:8px 0}.render-safety-pass{color:#166534}.render-safety-fail{color:#991b1b}';
    document.head.appendChild(s);
  }
  function toast(text) { $$('.render-safety-toast').forEach(e => e.remove()); const e = document.createElement('div'); e.className = 'render-safety-toast'; e.textContent = text; document.body.appendChild(e); setTimeout(() => e.remove(), 3200); }
  function closeModal() { $('render-safety-modal')?.remove(); }
  function modal(title, html, buttons = []) {
    closeModal();
    const box = document.createElement('div');
    box.id = 'render-safety-modal';
    box.className = 'render-safety-modal-backdrop';
    box.innerHTML = `<div class="render-safety-modal"><div class="render-safety-head"><h2>${esc(title)}</h2><button type="button" class="render-safety-close">×</button></div><div>${html}</div><div class="render-safety-actions"></div></div>`;
    const actions = box.querySelector('.render-safety-actions');
    buttons.forEach(x => { const b = document.createElement('button'); b.type = 'button'; b.textContent = x.label; b.addEventListener('click', x.onClick); actions.appendChild(b); });
    box.addEventListener('click', ev => { if (ev.target === box) closeModal(); });
    box.querySelector('.render-safety-close')?.addEventListener('click', closeModal);
  ...
  function activateTab(tab) {
    ...
    $$('.ribbon-tab', tabsBar).forEach(b => { const on = b.dataset.ribbonTab === tab; b.classList.toggle('active', on); b.setAttribute('aria-selected', on ? 'true' : 'false'); });
    mainToolbar.querySelectorAll('.tb-group').forEach(g => { const list = (g.dataset.ribbonTab || 'home').split(/\s+/); g.classList.toggle('ribbon-hidden', !list.includes(tab)); });
  ...
  function button(id, text, title, fn) {
    let b = $(id);
    if (!b) { b = document.createElement('button'); b.type = 'button'; b.id = id; b.textContent = text; b.title = title; }
  ...
  function ensureRenderTab() {
    ...
    $('btn-render-options-tab')?.remove();

    let tab = $('btn-render-tab');
    if (!tab) {
      tab = document.createElement('button');
      tab.type = 'button';
      tab.id = 'btn-render-tab';
      tab.className = 'ribbon-tab';
      tab.dataset.ribbonTab = 'render';
      tab.setAttribute('role', 'tab');
      tab.setAttribute('aria-selected', 'false');
      tab.title = 'אפשרויות רינדור';
      tab.textContent = 'רינדור';
      const collapse = $('ribbon-collapse-toggle');
      ...
      tab.addEventListener('click', ev => { ev.preventDefault(); ev.stopPropagation(); activateTab('render'); });
    }
    ...
    if (!panel) {
      ...
      const mainToolbar = document.querySelector('.ribbon-toolbar') || document.querySelector('.source-format-toolbar') || document.querySelector('.toolbar');
      if (mainToolbar) mainToolbar.after(panel); else renderBtn.parentElement?.after(panel);
    }
    const g1 = $('render-safety-render-group');
    const g2 = $('render-safety-diagnostics-group');
    const pause = $('btn-render-pause');
    if (g1 && pause && pause.parentElement !== g1) g1.appendChild(pause);
    if (g1 && !$('btn-render-stop-menu')) g1.appendChild(button('btn-render-stop-menu','■ עצור רינדור','עצור את הרינדור הנוכחי', stopRender));
    if (g2 && !$('btn-render-diagnostics')) g2.appendChild(button('btn-render-diagnostics','🔎 בדיקת רינדור','בדיקת מצב רינדור', diagnostics));
    if (g2 && !$('btn-ravtext-snapshots')) g2.appendChild(button('btn-ravtext-snapshots','⏪ שחזור','שחזור מגיבויים', snapshots));
    if (g2 && !$('btn-reset-display-only')) g2.appendChild(button('btn-reset-display-only','🧹 אפס תצוגה','איפוס הגדרות תצוגה', resetDisplay));
    if (localStorage.getItem('ravtext.ribbonTab') === 'render') activateTab('render');
  }
  ...
  function resetDisplay() { if (!confirm('לאפס רק הגדרות תצוגה ורינדור?\n\nהטקסט לא יימחק.')) return; snapshot('לפני איפוס תצוגה'); let n = 0; const rem = st => { RESET_KEYS.forEach(k => { if (st.getItem(k) !== null) { st.removeItem(k); n++; } }); const keys = []; for (let i = 0; i < st.length; i++) keys.push(st.key(i)); keys.forEach(k => { if (k && RESET_PREFIXES.some(p => k.startsWith(p))) { st.removeItem(k); n++; } }); }; try { rem(localStorage); rem(sessionStorage); } catch (_) {} toast('אופסו ' + n + ' הגדרות תצוגה.'); $('btn-render')?.click(); }
  ...
  function boot() { installStyle(); ensureRenderTab(); hookSnapshots(); }
