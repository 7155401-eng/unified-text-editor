(() => {
  ...
  function toast(text) {
    all('.render-safety-toast').forEach((el) => el.remove());
    const el = document.createElement('div');
    el.className = 'render-safety-toast';
    el.textContent = text;
  ...
  function clickRenderButtonForStop() {
    try { window.__ravtextCancelRender?.('render-safety-addons'); } catch (_) {}
    const btn = byId('btn-render');
    if (btn && /עצור|stop/i.test(btn.textContent || '')) btn.click();
    else {
      const status = byId('status');
      if (status) status.textContent = 'בקשת עצירת רינדור נשלחה.';
    }
  }
  ...
  function ensureButton(id, text, title, after, onClick) {
    ...
    if (btn && btn.dataset.renderSafetyHook !== '1') {
  ...
  function snapshotManager(){
    ...
    openModal('⏪ שחזור מגיבוי', rows, [
      {label:'צור גיבוי עכשיו', onClick:()=>{snapshot('גיבוי ידני'); snapshotManager();}},
      {label:'נקה גיבויים', onClick:()=>{if(confirm('למחוק גיבויים?')){localStorage.removeItem(SNAP_KEY); snapshotManager();}}}
    ]);
    byId('render-safety-modal')?.addEventListener('click',(ev)=>{
      const b=ev.target.closest('[data-snap]'); if(!b)return;
      const s=snaps().find(x=>x.id===b.dataset.snap); if(!s||!confirm('לשחזר? העמוד יתרענן.'))return;
  ...
  function hookSnapshots(){
    if(document.documentElement.dataset.renderSafetySnapshot==='1')return;
    document.documentElement.dataset.renderSafetySnapshot='1';
    document.addEventListener('click',(ev)=>{
      const b=ev.target.closest('button[data-cmd]'); const c=b?.dataset?.cmd;
      if(c&&dangerous.has(c)){snapshot('לפני פעולה: '+((b.textContent||c).trim())); toast('נוצר גיבוי אוטומטי.');}
    },true);
  }

  function resetDisplayOnly(){
    if(!confirm('לאפס רק הגדרות תצוגה ורינדור?\n\nהטקסט לא יימחק.'))return;
    snapshot('לפני איפוס תצוגה');
    let n=0;
    ...
    toast('אופסו '+n+' הגדרות תצוגה.');
    const r=byId('btn-render'); if(r) r.click();
  }
  function diagnostics(){
    const pg=byId('pages-container')||document.querySelector('.pages-container');
    const pc=pg?pg.querySelectorAll('.page:not(.page-placeholder)').length:0;
    const tal=localStorage.getItem('ravtext.talmudLayout')==='1';
    ...
    const checks=[
      ['מנוע',true,tal?'גפ״ת / V9':'רגיל'],
      ['רינדור',true,paused?'מושהה':(localStorage.getItem('ravtext.liveRender')==='0'?'כבוי':'פעיל')],
      ['עמודים',pc>0,String(pc)],
      ['פונטים',true,document.fonts?.status||'לא ידוע'],
      ['ביטול רינדור אמיתי',typeof window.__ravtextCancelRender==='function',typeof window.__ravtextCancelRender==='function'?'מותקן':'לא זוהה — כפתור עצור ינסה לעצור דרך הכפתור הראשי'],
      ['מפתחות ישנים',stale.length===0,stale.length?stale.join(', '):'נקי'],
      ['גיבויים',true,String(snaps().length)]
    ];
    openModal('🔎 בדיקת רינדור', checks.map(([a,ok,d])=>`<div class="render-safety-card ${ok?'render-safety-pass':'render-safety-fail'}">${ok?'✓':'✗'} <b>${esc(a)}</b> — ${esc(d)}</div>`).join(''), [
      {label:'רנדר עכשיו',onClick:()=>{closeModal(); byId('btn-render')?.click();}},
  ...
  function groupBlock(block){
    ...
    const kids=Array.from(block.children); if(kids.some(c=>c.classList?.contains('ravtext-stream-section')))return;
    block.innerHTML=''; const map=new Map();
    const help=document.createElement('span'); help.className='stream-settings-help'; help.textContent='הבהרה: בולד אמיתי הוא סימון B בעורך. דיבור המתחיל הוא הדגשה אוטומטית. “סגנון לבולד” רק מחליף איך טקסט שכבר מודגש נראה.'; block.appendChild(help);
    kids.forEach(el=>{ if(el.classList?.contains('stream-settings-code')||el.classList?.contains('stream-order-controls')){block.appendChild(el);return;} const k=bucket(el); if(!map.has(k)){const d=document.createElement('details');d.className='ravtext-stream-section';d.open=k==='בסיס'||k==='עיצוב ובולד';const s=document.createElement('summary');s.textContent=k;d.appendChild(s);map.set(k,d);} map.get(k).appendChild(el); });
    map.forEach(d=>block.appendChild(d)); block.classList.add('ravtext-grouped'); block.dataset.ravtextGrouped='1';
  }
  ...
  let pop=null;
  function info(el){ const se=el.closest?.('[data-stream]'); const code=se?.getAttribute('data-stream')||el.getAttribute?.('data-stream')||''; const s=(parseJson(localStorage.getItem('ravtext.streamSettings.v1')||'{}',{})||{})[code]||{}; const cs=getComputedStyle(el); return [['זרם',code||'לא זוהה'],['משקל פונט',cs.fontWeight],['גופן',cs.fontFamily],['גודל',cs.fontSize],['סגנון זרם',s.styleId||'—'],['סגנון שמחליף בולד',s.boldOverrideEnabled?(s.boldOverrideStyleId||'מופעל בלי סגנון'):'כבוי'],['דיבור המתחיל',s.lemmaBold===false?'כבוי':'פעיל']]; }
  ...
  function installProgressStop(){
    const host=byId('ravtext-render-progress-ui'); if(!host) return;
    const card=host.querySelector('.rtp-card')||host.firstElementChild||host;
    if(card && !host.querySelector('.rtp-cancel-addon')){
      const b=document.createElement('button'); b.type='button'; b.className='rtp-cancel-addon'; b.textContent='עצור'; b.title='עצור רינדור';
      b.addEventListener('click',(ev)=>{ev.preventDefault();ev.stopPropagation();clickRenderButtonForStop();host.remove();});
      card.appendChild(b);
    }
  }
