...
| כפתור פייתון | onclick | סטטוס | היכן בעורך החדש |
|---|---|---|---|
| `<input type="range" id="widthSlider">` (10..100) | `changeWidth(this.value)` | ❌ | חסר לחלוטין. אין סליידר רוחב כללי המכוון `flexBasis` לכל החלוניות. |
...
| מוסיף class `no-wrap` לכל `.ql-editor` (גלישת שורה כבויה) | ⚠️ |

החדש (`line_mode.js`) מפעיל רק class CSS שמשנה גלישה. אין בו מניפולציית טקסט אמיתית של הוספת/הסרת `\n` סביב סימני זרם.
