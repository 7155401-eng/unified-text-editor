...
export function wireComparatorButton(_paneManager) {
  // Generic data-cmd handler — picks up any existing button in the toolbar
  ...
  // Inject a default button if none exists yet — placed inside the
  // panes-toolbar (existing tools area) for visual consistency.
  if (!document.querySelector('[data-cmd="open-comparator"]')) {
    const toolsArea = document.querySelector('.panes-toolbar')
    ...
    wrap.innerHTML = `
      <button type="button" data-cmd="open-comparator" title="כלי עריכת הערות שוליים — השוואת מהדורות">📑 השוואת מהדורות</button>
      <button type="button" data-cmd="open-comparator-integrated" title="גרסת ממשק משתלב">📑 משתלב</button>
    `;
    toolsArea.appendChild(wrap);
  }
}
