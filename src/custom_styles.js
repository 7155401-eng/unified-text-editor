...
export function refreshCustomStylesGallery(select = document.getElementById("styles-gallery-select"), styles = loadStyles()) {
  ...
  for (const s of styles) {
    ...
    opt.value = s.id;
    opt.textContent = s.source === "docx" ? `${s.name} · Word` : s.name;
...
function refreshSavedList(dialog, paneManager, select) {
  ...
  for (const s of styles) {
    ...
    li.innerHTML = `<span class="csd-name-preview">${escapeHtml(s.source === "docx" ? `${s.name} · Word` : s.name)}</span><button type="button" data-action="edit" data-id="${escapeAttr(s.id)}">ערוך</button><button type="button" data-action="delete" data-id="${escapeAttr(s.id)}">מחק</button>`;
    ul.appendChild(li);
  }
  ul.onclick = ev => {
...
function styleOptions(selected = "") { return ['<option value="">בחר סגנון שמור…</option>', ...loadStyles().map(s => `<option value="${escapeAttr(s.id)}"${s.id === selected ? " selected" : ""}>${escapeHtml(s.source === "docx" ? `${s.name} · Word` : s.name)}</option>`)].join(""); }
...
function renderAutoRulesList(dialog, paneManager) { const ul = dialog.querySelector("#sar-list"); if (!ul) return; const state = loadAutoRulesState(); const locked = normalizeStreamCode(dialog.dataset.lockedStream || ""); const rows = locked ? state.rules.filter(r => normalizeStreamCode(r.stream) === locked) : state.rules; const streams = new Map(paneStreams(paneManager, locked).map(s => [s.value, s.label])); const styles = new Map(loadStyles().map(s => [s.id, s.source === "docx" ? `${s.name} · Word` : s.name])); ul.innerHTML = ""; if (!rows.length) { ul.innerHTML = '<li class="sar-empty">עדיין אין כללים לזרם הזה.</li>'; return; } for (const rule of rows) { const details = rule.type === "custom" ? ` · ${escapeHtml(rule.start)} ← ${escapeHtml(rule.end)}` : rule.type === "opening" ? ` · עד ${Number(rule.maxWords || 8)} מילים` : ""; const li = document.createElement("li"); li.innerHTML = `<span class="sar-rule-main"><b>${escapeHtml(streams.get(normalizeStreamCode(rule.stream)) || rule.stream)}</b> — ${escapeHtml(typeLabel(rule.type))}${details}<br><small>סגנון: ${escapeHtml(styles.get(rule.styleId) || rule.styleId || "חסר")}</small></span><button type="button" data-action="edit" data-id="${escapeAttr(rule.id)}">ערוך</button><button type="button" data-action="delete" data-id="${escapeAttr(rule.id)}">מחק</button>`; ul.appendChild(li); } }
...
export function wireCustomStyles(paneManager) {
  ...
  editBtn.type = "button";
  editBtn.className = "style-edit-selected-btn";
  editBtn.id = "btn-edit-selected-style";
  editBtn.textContent = "⋯";
  editBtn.title = "ערוך את הסגנון שנבחר מהרשימה";
  editBtn.setAttribute("aria-label", "ערוך את הסגנון שנבחר מהרשימה");
  editBtn.addEventListener("click", (ev) => { ev.preventDefault(); ev.stopPropagation(); openStyleEditor(select.value); });
  select.insertAdjacentElement("afterend", editBtn);
