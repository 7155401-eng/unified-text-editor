...
function buildDialog() {
  ...
  dialog.innerHTML = `
    ...
    <header class="csd-header"><span class="csd-title">סגנון משלך — הגדרות סגנון</span><button type="button" class="csd-close">✕</button></header>
    <div class="csd-body">
...
function refreshSavedList(dialog, paneManager, select) {
  ...
  for (const s of styles) {
    ...
    li.innerHTML = `<span class="csd-name-preview">${escapeHtml(s.name)}</span><button type="button" data-action="edit" data-id="${escapeAttr(s.id)}">ערוך</button><button type="button" data-action="delete" data-id="${escapeAttr(s.id)}">מחק</button>`;
    ul.appendChild(li);
  }
  ul.onclick = ev => {
    const btn = ev.target.closest("button"); if (!btn) return;
    const id = btn.dataset.id; const styles = loadStyles();
    if (btn.dataset.action === "delete") { const next = styles.filter(s => s.id !== id); saveStyles(next); refreshCustomStylesGallery(select, next); refreshSavedList(dialog, paneManager, select); refreshAutoRuleStyleOptions(); }
    if (btn.dataset.action === "edit") { const target = styles.find(s => s.id === id); if (target) { fillForm(target); dialog.dataset.editingId = id; } }
  };
}

function loadAutoRulesState() { try { const raw = JSON.parse(localStorage.getItem(AUTO_RULES_KEY) || "{}"); return { autoBeforeRender: !!raw.autoBeforeRender, rules: Array.isArray(raw.rules) ? raw.rules : [] }; } catch { return { autoBeforeRender: false, rules: [] }; } }
function saveAutoRulesState(state) { localStorage.setItem(AUTO_RULES_KEY, JSON.stringify({ autoBeforeRender: !!state.autoBeforeRender, rules: Array.isArray(state.rules) ? state.rules : [] })); }
...
function paneStreams(paneManager, extraStream = "") { const m = new Map(); for (const p of paneManager?.panes || []) m.set(streamKeyForPane(p), streamLabelForPane(p)); if (!m.has("MAIN")) m.set("MAIN", "ראשי"); const ex = normalizeStreamCode(extraStream); if (ex && ex !== "MAIN" && !m.has(ex)) m.set(ex, `זרם ${ex}`); return Array.from(m, ([value, label]) => ({ value, label })); }
function styleOptions(selected = "") { return ['<option value="">בחר סגנון שמור…</option>', ...loadStyles().map(s => `<option value="${escapeAttr(s.id)}"${s.id === selected ? " selected" : ""}>${escapeHtml(s.name)}</option>`)].join(""); }
function typeLabel(t) { return ({ opening: "דיבור המתחיל", bold: "טקסט בולד", round: "סוגריים עגולות", square: "סוגריים מרובעות", custom: "מקוד עד קוד" })[t] || t; }
...
function buildAutoRulesDialog(paneManager) {
  ...
  dialog.querySelector("#sar-new-rule")?.addEventListener("click", () => fillAutoRuleForm(dialog, { stream: dialog.dataset.lockedStream || "" }, paneManager));
  ...
  dialog.querySelector("#sar-apply-active")?.addEventListener("click", () => {
    const locked = dialog.dataset.lockedStream || "";
    const count = locked ? applyAutoStyleRulesForStream(paneManager, locked) : applyAutoStyleRules(paneManager, { scope: "active" });
    toastAutoRules(`הוחלו ${count} התאמות בזרם ${locked || "הפעיל"}.`);
  });
  ...
  dialog.querySelector("#sar-auto-render")?.addEventListener("change", ev => { const state = loadAutoRulesState(); state.autoBeforeRender = !!ev.target.checked; saveAutoRulesState(state); });
...
function refreshAutoRuleStreamOptions(dialog, paneManager, selected = "") { const sel = dialog?.querySelector("#sar-stream"); if (!sel) return; const locked = dialog?.dataset?.lockedStream || ""; const active = paneManager?.activePane ? streamKeyForPane(paneManager.activePane) : "MAIN"; const v = normalizeStreamCode(selected || locked || active); sel.innerHTML = paneStreams(paneManager, v).map(s => `<option value="${escapeAttr(s.value)}"${s.value === v ? " selected" : ""}>${escapeHtml(s.label)} (${escapeHtml(s.value)})</option>`).join(""); sel.value = v; sel.disabled = !!locked; }
function refreshAutoRuleStyleOptions(dialog = document.getElementById("stream-auto-rules-dialog"), selected = "") { const sel = dialog?.querySelector("#sar-style"); if (sel) sel.innerHTML = styleOptions(selected || sel.value); }
...
function fillAutoRuleForm(dialog, rule = {}, paneManager) { const locked = dialog.dataset.lockedStream || ""; dialog.dataset.editingRuleId = rule.id || ""; refreshAutoRuleStreamOptions(dialog, paneManager, rule.stream || locked); const set = (id, v) => { const el = dialog.querySelector(`#${id}`); if (el) el.value = v ?? ""; }; set("sar-type", rule.type || "opening"); refreshAutoRuleStyleOptions(dialog, rule.styleId || ""); set("sar-style", rule.styleId || ""); set("sar-start", rule.start || ""); set("sar-end", rule.end || ""); set("sar-max-words", rule.maxWords || 8); updateAutoRuleFormVisibility(dialog); }
function readAutoRuleForm(dialog) { const locked = dialog.dataset.lockedStream || ""; return { id: dialog.dataset.editingRuleId || "", stream: normalizeStreamCode(locked || dialog.querySelector("#sar-stream")?.value || "MAIN"), type: dialog.querySelector("#sar-type")?.value || "opening", styleId: dialog.querySelector("#sar-style")?.value || "", start: (dialog.querySelector("#sar-start")?.value || "").trim(), end: (dialog.querySelector("#sar-end")?.value || "").trim(), maxWords: Number(dialog.querySelector("#sar-max-words")?.value || 8) || 8 }; }
function saveAutoRuleFromForm(dialog, paneManager) { const rule = readAutoRuleForm(dialog); if (!rule.styleId) return alert("צריך לבחור סגנון שמור להחלה."); if (rule.type === "custom" && (!rule.start || !rule.end)) return alert("בכלל מסוג סימון-עד-סימון צריך למלא גם סימון פתיחה וגם סימון סיום."); const state = loadAutoRulesState(); if (rule.id) { const i = state.rules.findIndex(r => r.id === rule.id); if (i >= 0) state.rules[i] = rule; else state.rules.push({ ...rule, id: uniqueRuleId(state.rules) }); } else state.rules.push({ ...rule, id: uniqueRuleId(state.rules) }); saveAutoRulesState(state); renderAutoRulesList(dialog, paneManager); fillAutoRuleForm(dialog, { stream: dialog.dataset.lockedStream || rule.stream }, paneManager); }
function renderAutoRulesList(dialog, paneManager) { const ul = dialog.querySelector("#sar-list"); if (!ul) return; const state = loadAutoRulesState(); const locked = normalizeStreamCode(dialog.dataset.lockedStream || ""); const rows = locked ? state.rules.filter(r => normalizeStreamCode(r.stream) === locked) : state.rules; const streams = new Map(paneStreams(paneManager, locked).map(s => [s.value, s.label])); const styles = new Map(loadStyles().map(s => [s.id, s.name])); ul.innerHTML = ""; if (!rows.length) { ul.innerHTML = '<li class="sar-empty">עדיין אין כללים לזרם הזה.</li>'; return; } for (const rule of rows) { const details = rule.type === "custom" ? ` · ${escapeHtml(rule.start)} ← ${escapeHtml(rule.end)}` : rule.type === "opening" ? ` · עד ${Number(rule.maxWords || 8)} מילים` : ""; const li = document.createElement("li"); li.innerHTML = `<span class="sar-rule-main"><b>${escapeHtml(streams.get(normalizeStreamCode(rule.stream)) || rule.stream)}</b> — ${escapeHtml(typeLabel(rule.type))}${details}<br><small>סגנון: ${escapeHtml(styles.get(rule.styleId) || rule.styleId || "חסר")}</small></span><button type="button" data-action="edit" data-id="${escapeAttr(rule.id)}">ערוך</button><button type="button" data-action="delete" data-id="${escapeAttr(rule.id)}">מחק</button>`; ul.appendChild(li); } }
function openAutoRulesDialog(paneManager, streamCode = "") { const d = buildAutoRulesDialog(paneManager); const locked = normalizeStreamCode(streamCode); d.dataset.lockedStream = locked || ""; d.classList.toggle("sar-stream-locked", !!locked); const state = loadAutoRulesState(); const auto = d.querySelector("#sar-auto-render"); if (auto) auto.checked = !!state.autoBeforeRender; fillAutoRuleForm(d, { stream: locked || "" }, paneManager); renderAutoRulesList(d, paneManager); d.classList.add("open"); }
...
function wireStreamAutoStyleRules(paneManager, select) { installAutoRulesButton(paneManager, select); installAutoBeforeRenderHook(paneManager); installStreamRowButtons(paneManager); window.addEventListener("ravtext:styles-changed", () => refreshAutoRuleStyleOptions()); }
...
export function wireCustomStyles(paneManager) {
  ...
  select.addEventListener("change", ev => { const v = select.value; if (v === "__add-custom__") { ev.stopImmediatePropagation(); select.value = ""; openDialog(); return; } if (v) { ev.stopImmediatePropagation(); const target = loadStyles().find(s => s.id === v); select.value = ""; if (target) applyCustomStyleToActiveEditor(target, paneManager); } }, true);
