...
function streamKeyForPane(pane) { return pane?.streamCode ? String(pane.streamCode).padStart(2, "0") : "MAIN"; }
function streamLabelForPane(pane) { return pane?.streamCode ? (pane.label || `זרם ${pane.streamCode}`) : "ראשי"; }
function paneStreams(paneManager) { const m = new Map(); for (const p of paneManager?.panes || []) m.set(streamKeyForPane(p), streamLabelForPane(p)); if (!m.has("MAIN")) m.set("MAIN", "ראשי"); return Array.from(m, ([value, label]) => ({ value, label })); }
...
function buildAutoRulesDialog(paneManager) {
  ...
  dialog.innerHTML = `<div class="sar-backdrop"></div><div class="sar-window"><header class="sar-header"><span class="sar-title">כללי סגנון אוטומטיים לפי זרם</span><button type="button" class="sar-close">✕</button></header><div class="sar-body"><p class="sar-help">בחר זרם, בחר תבנית טקסט, ובחר סגנון שמור שיוחל על הטקסט שנמצא בתוך התבנית. אפשר להוסיף כמה כללי קוד מותאם שרוצים.</p><label class="sar-row"><span>זרם:</span><select id="sar-stream"></select></label><label class="sar-row"><span>על מה להחיל:</span><select id="sar-type"><option value="opening">דיבור המתחיל בתחילת פסקה</option><option value="bold">כל טקסט שכבר מסומן בולד</option><option value="round">טקסט בתוך סוגריים עגולות ( )</option><option value="square">טקסט בתוך סוגריים מרובעות [ ]</option><option value="custom">טקסט בין קוד פתיחה לקוד סיום</option></select></label><label class="sar-row"><span>סגנון להחלה:</span><select id="sar-style"></select></label><div class="sar-row sar-custom-row"><span>קודים:</span><input type="text" id="sar-start" placeholder="קוד פתיחה, למשל @11" /><input type="text" id="sar-end" placeholder="קוד סיום, למשל @22" /></div><label class="sar-row sar-opening-row"><span>דיבור המתחיל:</span><input type="number" id="sar-max-words" min="1" max="80" value="8" /><small>מילים לכל היותר, או עד נקודתיים/מקף/נקודה.</small></label><label class="sar-row"><span></span><label><input type="checkbox" id="sar-auto-render" /> החל את הכללים אוטומטית לפני רנדר</label></label><div class="sar-actions"><button type="button" class="primary" id="sar-save-rule">שמור כלל</button><button type="button" id="sar-new-rule">נקה טופס</button><button type="button" id="sar-apply-active">החל עכשיו על הזרם הפעיל</button><button type="button" id="sar-apply-all">החל עכשיו על כל הזרמים</button></div><div class="sar-list-wrap"><h4>כללים שמורים</h4><ul id="sar-list"></ul></div></div></div>`;
  document.body.appendChild(dialog);
  ...
  dialog.querySelector("#sar-new-rule")?.addEventListener("click", () => fillAutoRuleForm(dialog, {}, paneManager));
  dialog.querySelector("#sar-save-rule")?.addEventListener("click", () => saveAutoRuleFromForm(dialog, paneManager));
  dialog.querySelector("#sar-apply-active")?.addEventListener("click", () => toastAutoRules(`הוחלו ${applyAutoStyleRules(paneManager, { scope: "active" })} התאמות בזרם הפעיל.`));
...
function refreshAutoRuleStreamOptions(dialog, paneManager, selected = "") { const sel = dialog?.querySelector("#sar-stream"); if (!sel) return; const active = paneManager?.activePane ? streamKeyForPane(paneManager.activePane) : "MAIN"; const v = selected || active; sel.innerHTML = paneStreams(paneManager).map(s => `<option value="${escapeAttr(s.value)}"${s.value === v ? " selected" : ""}>${escapeHtml(s.label)} (${escapeHtml(s.value)})</option>`).join(""); }
...
function fillAutoRuleForm(dialog, rule = {}, paneManager) { dialog.dataset.editingRuleId = rule.id || ""; refreshAutoRuleStreamOptions(dialog, paneManager, rule.stream); const set = (id, v) => { const el = dialog.querySelector(`#${id}`); if (el) el.value = v ?? ""; }; set("sar-type", rule.type || "opening"); refreshAutoRuleStyleOptions(dialog, rule.styleId || ""); set("sar-style", rule.styleId || ""); set("sar-start", rule.start || ""); set("sar-end", rule.end || ""); set("sar-max-words", rule.maxWords || 8); updateAutoRuleFormVisibility(dialog); }
function readAutoRuleForm(dialog) { return { id: dialog.dataset.editingRuleId || "", stream: dialog.querySelector("#sar-stream")?.value || "MAIN", type: dialog.querySelector("#sar-type")?.value || "opening", styleId: dialog.querySelector("#sar-style")?.value || "", start: (dialog.querySelector("#sar-start")?.value || "").trim(), end: (dialog.querySelector("#sar-end")?.value || "").trim(), maxWords: Number(dialog.querySelector("#sar-max-words")?.value || 8) || 8 }; }
function saveAutoRuleFromForm(dialog, paneManager) { const rule = readAutoRuleForm(dialog); if (!rule.styleId) return alert("צריך לבחור סגנון שמור להחלה."); if (rule.type === "custom" && (!rule.start || !rule.end)) return alert("בכלל מסוג קוד מותאם צריך למלא גם קוד פתיחה וגם קוד סיום."); const state = loadAutoRulesState(); if (rule.id) { const i = state.rules.findIndex(r => r.id === rule.id); if (i >= 0) state.rules[i] = rule; else state.rules.push({ ...rule, id: uniqueRuleId(state.rules) }); } else state.rules.push({ ...rule, id: uniqueRuleId(state.rules) }); saveAutoRulesState(state); renderAutoRulesList(dialog, paneManager); fillAutoRuleForm(dialog, {}, paneManager); }
function renderAutoRulesList(dialog, paneManager) { const ul = dialog.querySelector("#sar-list"); if (!ul) return; const state = loadAutoRulesState(); const streams = new Map(paneStreams(paneManager).map(s => [s.value, s.label])); const styles = new Map(loadStyles().map(s => [s.id, s.name])); ul.innerHTML = ""; if (!state.rules.length) { ul.innerHTML = '<li class="sar-empty">עדיין אין כללים.</li>'; return; } for (const rule of state.rules) { const details = rule.type === "custom" ? ` · ${escapeHtml(rule.start)} ← ${escapeHtml(rule.end)}` : rule.type === "opening" ? ` · עד ${Number(rule.maxWords || 8)} מילים` : ""; const li = document.createElement("li"); li.innerHTML = `<span class="sar-rule-main"><b>${escapeHtml(streams.get(rule.stream) || rule.stream)}</b> — ${escapeHtml(typeLabel(rule.type))}${details}<br><small>סגנון: ${escapeHtml(styles.get(rule.styleId) || rule.styleId || "חסר")}</small></span><button type="button" data-action="edit" data-id="${escapeAttr(rule.id)}">ערוך</button><button type="button" data-action="delete" data-id="${escapeAttr(rule.id)}">מחק</button>`; ul.appendChild(li); } }
function openAutoRulesDialog(paneManager) { const d = buildAutoRulesDialog(paneManager); const state = loadAutoRulesState(); const auto = d.querySelector("#sar-auto-render"); if (auto) auto.checked = !!state.autoBeforeRender; fillAutoRuleForm(d, {}, paneManager); renderAutoRulesList(d, paneManager); d.classList.add("open"); }
...
function wireStreamAutoStyleRules(paneManager, select) { installAutoRulesButton(paneManager, select); installAutoBeforeRenderHook(paneManager); window.addEventListener("ravtext:styles-changed", () => refreshAutoRuleStyleOptions()); }
