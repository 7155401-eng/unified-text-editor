...
const LANG_ICON = '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15 15 0 0 1 0 20M12 2a15 15 0 0 0 0 20"/></svg>';

function syncLanguageSwitcher() {
  const btn = document.getElementById("langBtn");
  if (!btn) return;
  const nextLang = currentLang === "he" ? "en" : "he";
  const label = nextLang === "en" ? "EN" : "HE";
  btn.classList.add("language-switcher");
  btn.dataset.currentLang = currentLang;
  btn.dataset.nextLang = nextLang;
  btn.title = nextLang === "en" ? "English" : "עברית";
  btn.setAttribute("aria-label", nextLang === "en" ? "Switch language to English" : "החלף שפה לעברית");
  btn.innerHTML = `<span aria-hidden="true" style="display:inline-flex;vertical-align:middle;margin-inline-end:4px">${LANG_ICON}</span><b style="font-size:11px;letter-spacing:.04em">${label}</b>`;
