...
//   15. לולאת מתקנים עד יציבות (לא בזמנים קבועים; הלולאה אינה חוזרת אחורה —
//       מתחילה מהדף הראשון שיש בו שגיאה והלאה. דפים תקינים שלפניו לא נוגעים.)
//   16. שני כתרים מקבילים בכל 4 השורות
//
// הבהרת מינוח (חשוב!):
//   - "two-commentaries" כקלאס בקוד הזה = זרם הערות אחד שמתפצל לשני טורים מקבילים
//     (דפוס וילנא הקלאסי). זרימת הטקסט: מתחילת טור ימין → סוף → ראש טור שמאל → סוף.
//   - "two streams separately" = שני זרמי הערות נפרדים, כל אחד בטור משלו, אין זרימה ביניהם.
...
async function decideCrownMode(streams, hasMain, crownLines, halfWidthCss, fullWidthCss) {
  ...
  const res = await fetchEngineApi('/api/talmud/decide', {
    method: 'POST',
