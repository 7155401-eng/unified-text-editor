// render_controls.js — UI-level render pause/stop controls.
//
// This module intentionally works around the existing render pipeline without
// changing its engine contract: pause disables live render and resume triggers
// one final render; stop protects the current preview from being replaced by a
// stale in-flight render result.
...
function resumeRenderAndFlush() {
  ...
  // When the user exits pause mode, live rendering should be active unless it
  // had explicitly been off before pausing.
...
function installRenderButtonCapture() {
  ...
  renderBtn.addEventListener("click", (ev) => {
    ...
    // Manual render is allowed even when pause is active. Take a snapshot so
    // the stop button can restore the current preview if the user interrupts.
    snapshotCurrentOutput();
    state.running = true;
    updateRenderControlsUi();
  }, true);
...
function installRenderControls() {
  ...
  // Some toolbar elements are injected later by main.js. Re-run lightweight
  // wiring a few times without duplicating listeners.
  let tries = 0;
  const retry = () => {
    ensurePauseButton();
    installRenderButtonCapture();
