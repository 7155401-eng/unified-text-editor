...
export async function buildPagesDafLocked(container, paragraphs, cfg, opts = {}) {
  ...
  // ★ ההכרעה בין שתי הוראות של משה, 25/09:
  //   14/09 + 25/09 — "כל העמודים של המסמך באותו גודל, ביחס" (כי
  //                   בסוף מדפיסים ספר, וספר שדפיו בגדלים שונים
  //                   אינו ספר)
  //   24/09         — "אם יש רווח לבן בדף, הדף מתקטן"
  //
  // כשכמות התוכן שונה בכל דף וילנא אי אפשר לקיים את שתיהן: או גודל
  // אחיד (ואז דף דליל נשאר ריק למחצה), או גודל לכל דף (ואז אין ספר).
  // ההוראה האחרונה בזמן היא "כל העמודים של המסמך", ולכן היא הנבחרת
  // וזו ברירת המחדל.
  //
  // נמדד על הפלט של משה מ-25/09 (23 עמודים):
  //   גודל אחיד      — מילוי ממוצע 80.8%, הגרוע 56.3%, 9 מתחת ל-80%
  //   גודל לכל דף    — מילוי מלא, אבל 21 גדלים שונים (741–1261 פיקס׳)
  //
  // ⭐ ונמדד גם שהמכפיל האחיד אינו מנופח: הדף הצפוף ביותר מגיע
  //    למילוי 97.2%, כלומר הגודל נגזר ממנו במדויק ולא בהגזמה.
  //
  // מי שמעדיף מילוי מלא על פני אחידות יכול לבחור בכך:
  //   localStorage["ravtext.vilnaDaf.uniformPageSize"] = "0"
  // ⛔⛔⛔ תיקון הכרעה שגויה שלי (40708a7). משה, 25/09, תיאר את
  // התהליך הנדרש במפורש:
  //   "כל עמוד בודק את עצמו בלולאה עד שאין רווח בעמוד ולא דברים
  //    יוצאים מהעמוד, ואז מחיל את V9 ועובר לעמוד הבא. ואם אחרי V9
  //    עדיין יש רווח או תוכן יוצא — הוא שוב משנה את גודל העמוד
  //    בקנה מידה חדש (בהקפדה על יחסי גובה ורוחב) ומחיל את V9 עד
  //    להצלחה, ככה בלולאה."
  //
  // הלולאה הזו כבר קיימת למעלה, ב-fitPageToText: היא מחפשת לכל דף
  // בנפרד את המכפיל הקטן ביותר שבו הדף נכנס, משנה גובה ורוחב באותו
  // מכפיל, ובונה מחדש ב-V9 עד שמצליח.
  //
  // המעבר שמתחיל כאן **ביטל** אותה: הוא לקח את כל הדפים ויישר אותם
  // למכפיל של הדף הצפוף ביותר, ולכן כל דף דליל חזר להיות חצי ריק.
  // נמדד: מילוי ממוצע 80.8%, הגרוע 56.3%, 9 עמודים מתחת ל-80%.
  //
  // הבנתי קודם בטעות שמשה דורש **גודל** זהה לכל העמודים, והצגתי לו
  // סתירה שלא קיימת. הוא דרש **יחס** זהה — וזה בדיוק מה שהלולאה
  // עושה, כי היא מכפילה גובה ורוחב באותו מספר. יחס קבוע הוא שמאפשר
  // "קנה מידה סופי אחיד" בהדפסה, כפי שכתב ב-14/09.
  //
  // ⬛ המעבר נשאר בקוד במלואו ורק מדולג, ואפשר להחזירו בהגדרה אחת.
  const uniformPageSize = (() => {
    try { return localStorage.getItem("ravtext.vilnaDaf.uniformPageSize") === "1"; }
    catch (_) { return false; }
  })();
  if (settings.fitPageToText && uniformPageSize && allPages.length > 1) {
    let maxFactor = 0;
