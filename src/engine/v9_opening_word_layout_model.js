...
export const V9_OPENING_RESERVE_TOKEN = "__V9OPW__";
...
export function stripV9OpeningReserveTokens(text) {
  return String(text || "")
    .replace(new RegExp(`${V9_OPENING_RESERVE_TOKEN}[^\\s]*${V9_OPENING_RESERVE_TOKEN}`, "g"), "")
    .replace(/\s{2,}/g, " ")
    .trim();
}

export function makeV9OpeningReserveWord(segment) {
  const clean = String(segment || "").replace(/\s+/g, "");
  return `${V9_OPENING_RESERVE_TOKEN}${clean || "ממ"}${V9_OPENING_RESERVE_TOKEN}`;
}
...
export function buildV9OpeningWordLayoutModel(text, rawSettings, options = {}) {
  ...
  const cleanText = stripV9OpeningReserveTokens(String(text || ""));
  const parts = extractOpeningSegmentForTest(cleanText, settings);
  if (!parts || !parts.segment?.trim() || !parts.suffix?.trim()) return null;

  const position = "raised";
  const fontSizePx = options.baseFontSize
    ? (Number(options.baseFontSize) * settings.size) / 100
    : null;
