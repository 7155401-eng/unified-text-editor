...
// משה 2026-05-13: חילוץ טקסט פשוט מ-node (כל הצאצאים הטקסטואליים).
function textFromNode(node) {
  if (!node) return "";
  let text = "";
  node.descendants((child) => {
    if (child.isText) text += child.text;
    return true; // המשך לסרוק לעומק
  });
  return text;
}

// משה 2026-05-13: חילוץ מבנה של טבלה מ-node של TipTap.
// מחזיר מערך דו-מימדי: [[cell, cell, ...], [cell, cell, ...], ...]
// כל תא הוא טקסט פשוט (השלב הבא יוכל להוסיף תמיכה ב-marks).
...
function extractMainParagraphs(mainPane, paneManager) {
  ...
  if (symbols.length === 0) {
    ...
    mainPane.editor.state.doc.descendants((node) => {
      // משה 2026-05-13: נוסף 'table' לרשימה כדי שטבלאות לא ילכו לאיבוד.
      ...
      let tableRows = null;
      if (isTable) {
        tableRows = tableRowsFromNode(node);
      } else {
        node.descendants((child) => {
...
export function paneManagerToPackerContent(paneManager) {
  ...
  for (let pi = 0; pi < mainParagraphs.length; pi++) {
    ...
    paragraphsInfo.push({
      ...
      style: para.style || {},
      tableRows: para.tableRows || null, // משה 2026-05-13: העברת מבנה הטבלה
    });
  }
  ...
  for (const info of paragraphsInfo) {
    ...
    if (info.mainTextNet || cleanNotes.length || info.blockType === "table") {
      result.push({
        mainText: info.mainTextNet,
        notes: cleanNotes,
        blockType: info.blockType === "heading"
          ? "heading"
          : info.blockType === "table"
          ? "table"
          : "paragraph",
        headingLevel: info.blockType === "heading" ? Math.max(1, Math.min(6, info.headingLevel || 1)) : null,
        style: info.style || {},
        tableRows: info.blockType === "table" ? (info.tableRows || []) : null, // משה 2026-05-13
      });
    }
  }
