...
// ה-getter בוחר את המפתח הנכון לפי המצב, וערך ברירת מחדל מתאים.
...
function layoutTwoCommentariesWithMain(block, streamsWrap, mainEl, commentaryA, commentaryB) {
  ...
  if (!doCrown && !oneLongOneShort) {
    ...
    // Cloud-Claude 2026-05-06 + pass 143: שימוש בפונקציה האחידה
    // computeMinCrownHeight (כוללת FALLBACK_LINE_HEIGHT_PX). מבטל ה-pureLineHeight
    // המקומי שלא היה בו fallback קבוע, מה שהשאיר crowns ב-54/42px במקרי קצה.
...
export function wireTalmudLayoutControls(onChange) {
  ...
  // משה 2026-05-07: השדות הוזזו לסרגל "פריסה" (layout-extra-toolbar).
  // שמות talmud-* נשמרים לתאימות אחורה אם הם עדיין קיימים בעותק ישן.
  ...
  // משה: כשמשתנה מצב תלמוד, להתאים את ה-input לערך המצב החדש.
  toggle.addEventListener("change", () => {
    if (safetyInput) safetyInput.value = getEffectiveHeightSafety();
  });
