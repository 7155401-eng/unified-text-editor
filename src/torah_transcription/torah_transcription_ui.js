...
export class TranscriptionWindow {
  constructor(opts = {}) {
    ...
    // משה 2026-05-10: מצב ראשוני / שלב פתיחה — מאפשרים נקודות כניסה שונות
    // (תמלול אודיו, OCR, עריכה לשונית).
    this._initialMode = opts.initialMode || null; // "transcription" | "ocr" | null
    this._jumpToStepKey = opts.jumpToStep || null; // למשל "torah_style" לכניסה ישירה
    this._initialText = opts.initialText || ""; // טקסט מוכן לעריכה לשונית
    ...
    // משה 2026-05-10: מחיל מצב ראשוני אם נתון (OCR / טקסט מוכן)
    if (this._initialMode === "ocr") {
      this.appState.mode = "ocr";
    } else if (this._initialMode === "transcription") {
      this.appState.mode = "transcription";
    }
    if (this._initialText) {
      this.appState.result = { merged_text: this._initialText, edition: "manual" };
    }

    this._buildUi();
    // אם נתון jumpToStep — מתחילים שם, אחרת מהשלב הראשון
    let startIdx = 0;
    if (this._jumpToStepKey) {
      const visible = visibleSteps(this.appState);
      const found = visible.findIndex(([k]) => k === this._jumpToStepKey);
      if (found >= 0) startIdx = found;
    }
    this._showStep(startIdx);
  }
