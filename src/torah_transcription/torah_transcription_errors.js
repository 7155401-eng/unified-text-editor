...
export function friendlyError(errText) {
  ...
  if (s.includes('401') || s.includes('unauthorized') || s.includes('403') || s.includes('permission_denied')) {
    return {
      title: 'אין הרשאה',
      // נמצא בבדיקה 24/09: ההודעה אמרה מה קרה ולא מה לעשות.
      message: withDebugSuffix(
        'המפתח או החשבון לא הורשו לבצע את הפעולה הזו. שום דבר מהעבודה שלך לא נמחק.\n\n' +
        'מה לעשות:\n' +
        '• ודא שהמפתח הוזן בשדה של הספק הנכון ושלא פג תוקפו.\n' +
        '• ודא שהחשבון אצל הספק פעיל ומורשה לשירות הזה.\n' +
        '• אם הכול תקין, נסה שוב בעוד רגע.',
        raw),
    };
  }

  // ★ דיווח הלקוח (25–26/07/2026, צורף במייל): באמצע תמלול קפץ לו חלון
  //   "שגיאה — תשובה לא תקינה מהשרת server_error".
  //
  // למה זה קרה: לכל סוג שגיאה כאן יש הודעה אנושית — חוץ מ-server_error,
  // שאין לו מקרה משלו. לכן הוא נפל למקרה האחרון, וזה החזיר את הטקסט
  // הגולמי כמו שהוא: קוד באנגלית ומשפט חצי-טכני, בלי לומר מה קרה, בלי
  // לומר אם העבודה אבדה, ובלי לומר מה לעשות עכשיו.
  if (
    s.includes('server_error') ||
    s.includes('תשובה לא תקינה') ||
    s.includes('non-json') ||
    s.includes('unexpected token')
  ) {
    return {
      title: 'השרת החזיר תשובה שלא הצלחנו לקרוא',
      message: withDebugSuffix(
        'הפעולה לא הושלמה, ושום דבר מהעבודה שלך לא נמחק.\n\n' +
        'מה לעשות עכשיו:\n' +
        '• נסה שוב בעוד רגע — לרוב זו תקלה חולפת.\n' +
        '• אם אתה מאחורי סינון רשת, ייתכן שהסינון החזיר דף במקום התשובה.\n' +
        '• אם זה חוזר, שלח לנו את מזהה השגיאה שלמטה.',
        raw),
    };
  }

  // מקרה אחרון: עדיין אומרים מה קרה ומה לעשות, והפרט הטכני נשאר בסוף
  // כדי שאפשר יהיה לשלוח אותו — אבל הוא כבר לא כל ההודעה.
  return {
    title: 'משהו השתבש',
    message: withDebugSuffix(
      'הפעולה לא הושלמה, ושום דבר מהעבודה שלך לא נמחק.\n\n' +
      'נסה שוב בעוד רגע. אם זה חוזר, שלח לנו את השורה הזו:\n' +
      (raw || '(אין פרטים נוספים)'),
      raw),
  };
}
