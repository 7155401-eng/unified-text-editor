...
## 2026-05-21 - [spacing_settings.js Cache Optimization]
**Learning:** Similar to text styles, the spacing settings were repeatedly calling synchronous I/O (`localStorage.getItem`) and expensive parsing (`JSON.parse`) via inline `localStorage` lookups embedded deep within layout algorithms like `dom_packer.js`.
**Action:** Implemented an in-memory cache variable backed by `localStorage` within `spacing_settings.js`, and replaced distributed `localStorage.getItem` queries with the centralized `loadSpacingSettings()` cached helper. This centralizes caching and safely updates cross-tab listeners via the `storage` event.
